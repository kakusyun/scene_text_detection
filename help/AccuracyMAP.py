...
def cal_mAP(gt_dir, predict_dir):
    ...
    """
    tmp_files_path = gt_dir[:-13] + 'mAP_' + predict_dir[-28:] + "/tmp_files"
    if not os.path.exists(tmp_files_path):  # if it doesn't exist already
        os.makedirs(tmp_files_path)
    results_files_path = gt_dir[:-13] + 'mAP_' + predict_dir[-28:] + "/results"
    ...
    for txt_file in ground_truth_files_list:
        ...
        if not os.path.exists(predict_dir + file_id + ".txt"):
    ...
    predicted_files_list = glob.glob(predict_dir + '*.txt')
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for txt_file in predicted_files_list:
            ...
            file_id = os.path.basename(os.path.normpath(file_id))
            if class_index == 0:
                if not os.path.exists(gt_dir + file_id + ".txt"):
    ...
    #         ""
    #     )
    ap_dictionary.update({'mAP': mAP})
    return ap_dictionary


def CalAccuracy(gt_dir, predict_dir, today, mn):
...
def startCalACC(result_path, model_name):
    ...
    acc_result = CalAccuracy(gt_dir, predict_dir, end_date, model_name)
    print(end_date)

    sys.path.append('./help')

    ap_result = cal_mAP(gt_dir, predict_dir)
    result = dict(acc_result, **ap_result)
