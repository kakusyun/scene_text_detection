...
class DataAugmentForObjectDetection():
    ...
    def _cutout(self, img, bboxes, length=50, n_holes=1, threshold=0.5):
        ...
        for n in range(n_holes):
            ...
            while chongdie:
                ...
                y1 = np.clip(y - length // 2, 1, h)
                y2 = np.clip(y + length // 2, 1, h)
                x1 = np.clip(x - length // 2, 1, w)
                x2 = np.clip(x + length // 2, 1, w)
    ...
    def _rotate_img_bbox(self, img, bboxes, angle=5, scale=1.):
        ...
        for bbox in bboxes:
            ...
            # 加入list中
            rot_bboxes.append([rx_min, ry_min, rx_max, ry_max, bbox[4], bbox[5]])
    ...
    def _filp_pic_bboxes(self, img, bboxes):
        ...
        for box in bboxes:
            ...
            if horizon:
                flip_bboxes.append([w - x_max + 1, y_min, w - x_min + 1, y_max, box[4], box[5]])
            else:
                flip_bboxes.append([x_min, h - y_max + 1, x_max, h - y_min + 1, box[4], box[5]])

        return flip_img, flip_bboxes

    def checkbbox(self, bboxes):
        for bb in range(len(bboxes)):
            if bboxes[bb][0] == 0 or bboxes[bb][1] == 0:
...
if __name__ == '__main__':
    ...
    source_pic_root_path = '../../dataset/VOCtrainval_11-May-2012/VOCdevkit' \
                           '/VOC2012/JPEGImages/'
    source_xml_root_path = '../../dataset/VOCtrainval_11-May-2012/VOCdevkit/VOC2012/Annotations/'
    destination_xml_path = '../../dataset/VOCtrainval_ImgAug_by_kaku/Annotations/'
    destination_pic_path = '../../dataset/VOCtrainval_ImgAug_by_kaku/JPEGImages/'
    if not os.path.exists(destination_pic_path):
        os.makedirs(destination_pic_path)
    if not os.path.exists(destination_xml_path):
        os.mkdir(destination_xml_path)

    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            cnt = 0
            while cnt < need_aug_num:
                d_pic_path = destination_pic_path + str(cnt).zfill(2)
                if not os.path.exists(d_pic_path):
                    os.mkdir(d_pic_path)
                d_xml_path = destination_xml_path + str(cnt).zfill(2)
                if not os.path.exists(d_xml_path):
                    os.mkdir(d_xml_path)

                pic_path = os.path.join(parent, file)
                xml_path = os.path.join(source_xml_root_path, file[:-4] + '.xml')
                coords = xml_helper.parse_xml(xml_path)  # 解析得到box信息，格式为[[x_min,y_min,x_max,y_max,name,difficult]]
                # print(coords)
                ...
                for i in range(len(coords)):
                    if coords[i][0] == 0 or coords[i][1] == 0:
                        ...
                        print(coords[i])
                        os._exit(0)
                    if coords[i][2] > img.shape[1] or coords[i][3] > img.shape[0]:
                        print('Mistake happens.')
                        print(coords[i])
                        os._exit(0)
                ...
                # 写入新的XML
                xml_helper.generate_xml(img_name, auged_bboxes, img_size, d_xml_path)

                # 保存增强后的图像
                # auged_img_filename = os.path.join(d_pic_path, img_name)
                # cv2.imwrite(auged_img_filename, auged_img)
